...
public class UserControllerImpl implements UserController {
	...
	public String insertUser2(@ModelAttribute("snsUser") Map<String, Object> snsUser, LoginDTO loginDTO,
		...
		loginDTO.setId((String) snsUser.get("email"));
		loginDTO.setPassword((String) snsUser.get("id"));
		String hashedPw = BCrypt.hashpw((String) snsUser.get("id"), BCrypt.gensalt(10));
		snsUser.put("id", hashedPw);
	...
	logger.info("로그인끗");return"redirect:/user/logIn.do";
	...
	public ModelAndView logIn(HttpServletRequest request, LoginDTO loginDTO, HttpSession httpSession, ModelAndView mav)
		...
		if (userVO == null || !BCrypt.checkpw(loginDTO.getPassword(), userVO.getPassword())) {
			System.out.println("널...... 가쥘쑤 이쓸꺼라 쉥각해쒀");
		...
		System.out.println("도대체 어디로 간거니?2");
		System.out.println("유저컨트롤러mav" + mav);
		// 로그인 유지를 선택할 경우
		if (loginDTO.isUseCookie()) {
			int amount = 60 * 60 * 24 * 7; // 7일
			Date sessionLimit = new Date(System.currentTimeMillis() + (10000 * amount));// 로그인 유지기간 설정
